...
const makeMessagesSocket = (config) => {
    ...
    const shade = new asukoe(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = shade.detectType(content);
            ...
            if (messageType) {
                switch(messageType) {
                    case 'PAYMENT':
                        const paymentContent = await shade.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await shade.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await shade.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        return await shade.handleAlbum(content, jid, quoted)
                    case 'EVENT':
                        return await shade.handleEvent(content, jid, quoted)
                    case 'POLL_RESULT':
                        return await shade.handlePollResult(content, jid, quoted)
                    case 'GROUP_STORY':
                        return await shade.handleGroupStory(content, jid, quoted)
                }
            }
